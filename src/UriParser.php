...
class UriParser
{
    ...
    const FILTER_QUERY_PATTERN = '/filter={([^}]+)}/';

    const REMOVE_FILTER_FROM_URI = '/(filter={[^}]+})/';
    ...
    {
        $queryFormatted = $this->setFilterParameters($queryUri);

        $queryParameters = array_filter(explode('&', $queryFormatted));

        array_map([$this, 'appendQueryParameter'], $queryParameters);
    }

    private function setFilterParameters($queryUri)
    {
        preg_match(self::REMOVE_FILTER_FROM_URI, $queryUri, $matches);

        if (count($matches) > 0) {
            preg_match(self::FILTER_QUERY_PATTERN, $matches[1], $fields);
            
            $fields[1] = str_replace(',', '&', $fields[1]);

            $queryUri = str_replace($matches[1], $fields[1], $queryUri);
        }
